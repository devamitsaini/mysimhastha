      ...
      import HomeGuides from "../components/common/HomeGuides";
  ...
  if (nriForm.country.length > 100) {
    alert("Country name too long");
    return;
  }
  ...
  e.key === "Enter" && navigate(`/news/${n.slug}`)
      ...
      Read More →
        ...
        {KUMBH_YEARS.map((k) => (
          <div
            ...
            onClick={() => k.active && navigate('/simhastha-2028')}
          >
    ...
    >
          ...
          View All Cases →
                ...
                <div className="mp-grid">
                  {mpList.slice(0, 2).map((p) => (
                    <div key={p.id} className="mp-card" onClick={() => navigate('/missing-persons')} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && navigate('/missing-persons')}>
      ...
      }} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && navigate('/hotels')}>
